...
We have a processed MS dataset of analytes measured for different bacteria at different time-points with different dosages of an antibiotic. 

```{r 7203, echo=FALSE, fig.cap="\\label{fig:5202}Checking our messy data table."}
knitr::include_graphics("images/bacteria-table.png")
```
We can see multiple columns, some expanded excel columns for metabolites. A lot of messiness.

Our goal will be to:
* Read excel data with `readxl` library
* Have `Bacteria`, `Dosage`, `Metabolite`, `Time_Point`, `Response` be our columns
...
```{r 7202, echo=FALSE, fig.cap="\\label{fig:7203}Separating columns in a tibble."}
...
```{r 7202, echo=FALSE, fig.cap="\\label{fig:7204}Mutate:Making new columns or changing existing ones."}
...
```{r 7202, echo=FALSE, fig.cap="\\label{fig:7205} Pivoting longer from wide data."}
...
```{r}
tidy_met_df %>%
  group_by(culture, dose_mg, metabolite, time_min) %>%
