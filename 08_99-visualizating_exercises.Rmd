...
- Create a new R Studio Project and name it *006_data_visualizing*.
...
4. ... color by `min_temp`, creating a color scheme of low = "lightblue" and high = "orange", mid = "yellow"...
```{r, echo=FALSE, fig.width=4, fig.height=3, fig.align='center'}
tbl_den %>% 
  ggplot(aes(precipitation, snowfall)) + 
...
5. ... add a smoothed line under the points colored "grey", facet by `year` keeping all plots on a single line, and change the theme to have a dark background ...
...
tbl_den %>%
  ...
  ggplot() +
  geom_boxplot(aes(month, temp, group = temp_ends), fill=NA, color='grey30', position = 'identity') +
